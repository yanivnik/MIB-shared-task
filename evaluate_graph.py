...
def make_input_construction_hook_(model: HookedTransformer, activation_matrix, in_graph_vector, neuron_matrix, attention_head_mask=None, non_attention_head_mask=None, attention_biases=None):
...
def evaluate_graph(model: HookedTransformer, graph: Graph, dataloader: DataLoader, metrics: Union[Callable[[Tensor],Tensor], List[Callable[[Tensor], Tensor]]], quiet=False, intervention: Literal['patching', 'zero', 'mean','mean-positional']='patching', intervention_dataloader: Optional[DataLoader]=None, means: Optional[Tensor]=None, corrupted_activations: Optional[List[Tensor]]=None) -> Union[torch.Tensor, List[torch.Tensor]]:
    ...
    Args:
        ...
        corrupted_activations: Optional[List[Tensor]]: The corrupted activations to use for the patching intervention, if you have them precomputed. If not provided, they will be computed from the dataloader.
    ...
    attention_head_mask, non_attention_head_mask, attention_biases = None, None, None
    if model.cfg.use_normalization_before_and_after:
    ...
    make_input_construction_hook = partial(make_input_construction_hook_, model, attention_head_mask=attention_head_mask, non_attention_head_mask=non_attention_head_mask, attention_biases=attention_biases)

    def make_input_construction_hooks(activation_differences, in_graph_matrix, neuron_matrix):
        input_construction_hooks = []
        for layer in range(model.cfg.n_layers):
            # If any attention node in the layer is in the graph and we're in neuron-mode or at least one parent edge is not in graph 
            # (corruption needed) just construct the input for the entire layer
            ...
            # add MLP hook if MLP in graph and we're in neuron-mode or at least one parent edge is not in graph (corruption needed)
        ...
        # Always add the logits hook if  we're in neuron-mode or at least one parent edge is not in graph (corruption needed)
    ...
    for i, (clean, corrupted, label) in enumerate(dataloader):
        ...
        with torch.inference_mode():
            if intervention == 'patching':
                if corrupted_activations is None:
                    # We intervene by subtracting out clean and adding in corrupted activations
                    with model.hooks(fwd_hooks_corrupted):
                        corrupted_logits = model(corrupted_tokens, attention_mask=attention_mask)
                else:
                    # We've precomputed the corrupted activations, so we just need to add them in
                    activation_difference += corrupted_activations[i]#.to(activation_difference.device)
            else:
            ...
            clean_logits = None # model(clean_tokens, attention_mask=attention_mask)
