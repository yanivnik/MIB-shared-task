...
class Graph:
    ...
    def apply_node_topn(
        ...
        """
        Gets the topn nodes of the graph using a greedy algorithm that works from the scores up.
        Should run node attribution prior.
        """
        assert self.nodes_scores is not None
        if n_nodes > len(self.nodes):
            raise ValueError(
                f"n ({n_nodes}) is greater than the number of nodes ({len(self.nodes)})"
            )
        ...
        chosen_node_indices = abs_id(self.nodes_scores).topk(n_nodes).indices
        self.nodes_in_graph[chosen_node_indices] = True

        # Add all edges between the attributed nodes (used to be able to use the same eval functions)
        self.in_graph[chosen_node_indices] = True
    ...
    def to_networkx(self, check_in_graph: bool = False):
        ...
        orig_nodes, orig_edges = len(self.nodes), len(self.edges)

        # Add nodes to networkx graph
        for ind, (name, node) in enumerate(self.nodes.items()):
        ...
        if not check_in_graph:
            assert len(G.nodes) == len(self.nodes) and len(G.edges) == len(
                self.edges
            ), f"Networkx graph has {len(G.nodes)} nodes and {len(G.edges)} edges, but original graph had {orig_nodes} nodes and {orig_edges} edges"
        return G
    ...
    def from_json(cls, json_path: str) -> "Graph":
        ...
        with open(json_path, "r") as f:
            d = json.load(f)
            assert all(
                [k in d.keys() for k in ["cfg", "nodes", "edges"]]
